...
use ZipArchive;
class SysDataModel extends CommonModel {
    ...
    public function bakupTable($table_list) {
        ...
        $this->execute("SET SQL_QUOTE_SHOW_CREATE = 1"); //1，表示表名和字段名会用``包着的,0 则不用``
        ...
        foreach ($table_list as $table) {
            ...
            $tmp = $this->query("SHOW CREATE TABLE {$table}"); 
    ...
    public function getSqlFilesList() {
        ...
        while ($file = readdir($handle)) {
            if (preg_match('#\.sql$#i', $file)) {
                $fp = fopen(C('DatabaseBackDir') . "/$file", 'rb');
                ...
                $_size = filesize(C('DatabaseBackDir') . "/$file");
                ...
                $mtime = filemtime(C('DatabaseBackDir') . "/$file");
                $list[$mtime][$file] = $bk;
            }
        }
        closedir($handle);
    ...
    public function getZipFilesList() {
        $list = array();
        $size = 0;
        $handle = opendir( INCLUDE_PATH."Zip/");

        while ($file = readdir($handle)) {
            if ($file != "." && $file != "..") {
                $tem = array();
                $tem['file'] = $file; //  checkCharset($file);
                $_size = filesize( INCLUDE_PATH."Zip/$file");
                $tem['size'] = byteFormat($_size);
                $tem['time'] = date("Y-m-d H:i:s", filectime( INCLUDE_PATH."Zip/$file"));
                $size+=$_size;
                $list[] = $tem;
            }
        }
     ...
     * @return array
      +----------------------------------------------------------
     */
    public function zip($files, $filename, $outDir = WEB_CACHE_PATH, $path = "./Include/backup") {
    ...
    function unzip($file, $outDir = '') {
        $zip = new ZipArchive();
        if ($zip->open( INCLUDE_PATH."Zip/" . $file) !== TRUE)
            return FALSE;
