...
class LangModel extends CommonModel {
	...
	public function updateLangCache($lang_id, $mark) {
		...
		$file_wap_name = APP_PATH  . "Wap/Lang/" . $mark . ".php";
		...
		if (! file_exists_case ( $file_wap_name )) {
			$myfile = fopen ( $file_wap_name, "w" );
			fwrite ( $myfile, $config );
			fclose ( $myfile );
			chmod ( $file_wap_name, 0777 );
		} else {
			file_put_contents ( $file_wap_name, $config );
		}
