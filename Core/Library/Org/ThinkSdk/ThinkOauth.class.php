...
// +----------------------------------------------------------------------
// | ThinkOauth.class.php 2013-12-24
// +----------------------------------------------------------------------
...
abstract class ThinkOauth {
	...
	public function __construct($token = null) {
		...
		$this->login_info = $login_api_model->getInfoByTypeName ( $this->Type );
		if (empty ( $this->login_info ['appkey'] ) || empty ( $this->login_info ['appsecret'] )) {
			throw_exception ( '请配置您申请的APP_KEY和APP_SECRET' );
		} else {
			$this->AppKey = $this->login_info ['appkey'];
			$this->AppSecret = $this->login_info ['appsecret'];
			$this->Token = $token; //设置获取到的TOKEN
			if (! empty ( $this->login_info ['AUTHORIZE'] )){
				$this->Authorize = $this->login_info ['AUTHORIZE'];
			}
			if (! empty ( $this->login_info ['callback'] )){
				$this->Callback = $this->login_info ['callback'];
			}
		}
	}
	...
	public static function getInstance($type, $token = null) {
		...
		$filename = LIB_PATH . "Org/ThinkSdk/{$name}.class.php";
