...
/**
    ...
    static public function getAccessList($authId) {
        ...
        $sql    =   "select node.id,node.name from ".
                    ...
                    $table['node']." as node ".
                    "where user.user_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=4 and node.status=1";
        $apps =   $db->query($sql);  
        $access =  array();
        foreach($apps as $key=>$app) {
            ...
            foreach($modules as $key=>$module) {
                ...
                // 和公共模块的操作权限合并
                $action += $publicAction;
                if(isset($access[strtoupper($appName)][strtoupper($moduleName)])){
                	 $access[strtoupper($appName)][strtoupper($moduleName)]=array_merge( $access[strtoupper($appName)][strtoupper($moduleName)],array_change_key_case($action,CASE_UPPER));
                }else{
