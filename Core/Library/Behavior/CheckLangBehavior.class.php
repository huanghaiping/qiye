...
class CheckLangBehavior {
	...
	private function checkLanguage() {
		...
		if (! $langList) {
			$langList = D ( ADMIN_NAME."/Lang" )->getLang();
			C ( 'LANG_LIST', implode ( ",", getSubByKey ( $langList, "mark" ) ) );
		}
		//是否启用域名绑定语言
		$domain_lang = C ( 'DEFAULT_LANG' );
		if (C ( 'DOMAIN_SWITCH_ON', null, false )&&MODULE_NAME=="Home") {
			$http_host = $_SERVER ['HTTP_HOST'];
			$lang_array = F ( 'Lang', '', INCLUDE_PATH );
			foreach ( $lang_array as $key => $value ) {
				$domain = strpos ( $value ['domain'], "," ) ? explode ( ",", $value ['domain'] ) : array ($value ['domain'] );
				if (in_array ( $http_host, $domain )) {
					$domain_lang = $key;
					cookie ( 'think_language', $domain_lang, 3600 );
					break;
				}
			}
		}
		...
		if (C ( 'LANG_AUTO_DETECT', null, true )) {
			...
			} elseif (isset ( $_SERVER ['HTTP_ACCEPT_LANGUAGE'] ) && C ( 'BROWSER_SWITCH_ON', null, false )) { // 自动侦测浏览器语言
